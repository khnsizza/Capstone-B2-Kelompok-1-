        ...
        results.append(
            {
                ...
                "server_latency_ms": body.get("latency_ms", "-"),  # server-reported
...
async def run_scenario(base_url: str, runs: int) -> None:
    ...
    async with httpx.AsyncClient() as client:
        for run in range(1, runs + 1):
            ...
            print(f"  [Skenario 4.1] COLD CACHE — stampede prevention")
            print(f"  Target: Nasabah 1 ~700ms (DB), gap Nasabah 2-10 < 50ms")
            print(f"{'='*60}")

            # Reset server-side counters
            await reset_metrics(client)

            # Always flush Redis — cold cache / stampede scenario every run
            ...
            print(f"\n{'ID':>3}  {'HTTP':>4}  {'HTTP-ms':>8}  {'Fresh-ms':>9}  {'Source':<18}  Details")
            print("-" * 74)
            for r in results:
                srv = r.get('server_latency_ms', '-')
                srv_str = f"{srv:>9}" if isinstance(srv, int) else f"{'?':>9}"
                if r["source"] == "error":
                    print(
                        f"{r['id']:>3}  {r['status_code']:>4}  {r['latency_ms']:>8.1f}  {srv_str}  "
            ...
            print(f"  * HTTP-ms  = total round-trip from client (Python)")
            print(f"  * Fresh-ms = Nasabah1: DB query time | Nasabah2-10: ms since cache was written (target <10ms)")
